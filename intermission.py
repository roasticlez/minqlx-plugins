...
SONGS = [
    "sound/allah/slayer-angel_of_death.ogg"    
]
...
class intermission(minqlx.Plugin):
    def __init__(self):
        ...
        self.add_command("songs", self.cmd_enable_songs, usage="<0/1>", client_cmd_perm=0)


    def cmd_intermission( self, player, msg, channel):
        for p in self.players():
            if self.db.get_flag(p, "intermission:songs_enabled", default=True):
                self.stop_sound(p)
    ...
    def cmd_enable_songs(self, player, msg, channel):
      flag = self.db.get_flag(player, "intermission:songs_enabled", default=True)
      self.db.set_flag(player, "intermission:songs_enabled", not flag)
      
      if flag:
          player.tell("Songs have been disabled. Use ^6{}songs^7 to enable them again."
              .format(self.get_cvar("qlx_commandPrefix")))
      else:
          player.tell("Songs have been enabled. Use ^6{}songs^7 to disable them again."
              .format(self.get_cvar("qlx_commandPrefix")))
